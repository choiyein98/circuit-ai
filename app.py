...
# ==========================================
# [설정] BrainBoard V61: Memory Optimized
# ==========================================
st.set_page_config(page_title="BrainBoard V61: Optimized", layout="wide")

# [핵심 변경] 모델 3개 로딩 -> 1개로 축소 (메모리 절약)
# 가장 최신/성능 좋은 파일 하나만 선택하세요.
...
# ==========================================
# [분석 2] 실물 보드 (Real Board) - Single Model
# ==========================================
...
# [Main UI]
# ==========================================
st.title("🧠 BrainBoard V61: Optimized & Stable")
st.markdown("### ⚡ 메모리 최적화 + 회로이론 정밀 검증")

@st.cache_resource
def load_models():
    # 메모리 절약을 위해 가비지 컬렉션 수행
    gc.collect()
    return YOLO(REAL_MODEL_PATH), YOLO(MODEL_SYM_PATH)

try:
    model_real, model_sym = load_models()
    st.sidebar.success("✅ 모델 로드 완료 (Light Mode)")
...
if ref_file and tgt_file:
    ...
    if st.button("🚀 분석 실행"):
        with st.spinner("최적화된 모드로 분석 중..."):
            ...
            st.image(cv2.cvtColor(res_tgt_img, cv2.COLOR_BGR2RGB), caption="실물 분석 (메모리 최적화됨)", use_column_width=True)
