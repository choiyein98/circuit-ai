...
# ==========================================
# [설정 및 상수] 모델 파일명 최신화 (V46)
# ==========================================
st.set_page_config(page_title="BrainBoard V46: Final System", layout="wide")

# [중요] GitHub에 업로드된 파일명과 100% 일치해야 합니다!
# 실물 모델: 방금 올리신 가장 최신 파일 사용
MODEL_REAL_PATH = 'best(3).pt'
# 회로도 모델: 아까 올리신 회로도 전용 파일 사용
...
# [신뢰도 설정] 부품별 민감도 미세 조정
CONFIDENCE_MAP = {
    'led': 0.60,
    'capacitor': 0.45,
    'voltage': 0.25,
    'source': 0.25,
    'resistor': 0.50, # 저항은 50% 이상만 인정
    'wire': 0.35,
    'default': 0.35
}

# ==========================================
# [Helper Functions] V44 정밀 로직 유지
# ==========================================
...
def is_valid_size(box, img_w, img_h):
    ...
    if area < img_area * 0.001: return False # 0.1% 미만 제거
    return True

# [Resistor Filter] 거대 박스 제거
def is_valid_resistor_size(box, img_w, img_h):
    ...
    area = w * h
    img_area = img_w * img_h
    if area > img_area * 0.05: return False # 5% 초과 제거
    return True
...
# ==========================================
# [분석 1] 회로도 (Schematic) - symbol.pt 사용
# ==========================================
...
# [Main UI]
# ==========================================
st.title("🧠 BrainBoard V46: Final System")
st.markdown("### PSpice 회로도와 실제 브레드보드 사진을 업로드하세요.")
st.caption("✅ Final Model Applied: `best(3).pt` & `symbol.pt`")
...
if ref_file and tgt_file:
    ...
    if st.button("🚀 회로 검증 시작 (Analyze)"):
        with st.spinner("최신 AI 모델로 정밀 분석 중..."):
            ...
            st.image(cv2.cvtColor(res_ref_img, cv2.COLOR_BGR2RGB), caption="PSpice 회로도 분석 (Symbol Model)", use_column_width=True)
            st.image(cv2.cvtColor(res_tgt_img, cv2.COLOR_BGR2RGB), caption="실물 보드 분석 (Real Model V46)", use_column_width=True)
